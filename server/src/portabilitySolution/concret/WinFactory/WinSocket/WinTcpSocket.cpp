...
#include "WinTcpSock.hpp"

void		WinTcpSock::connect(void)
{
...
void		WinTcpSock::send(void *pack, int size)
{
  WSABUF	dataBuf;
  DWORD		sendBytes;
...
void		WinTcpSock::bind(void)
{
  if (bind(this->_sock, static_cast<SOCKADDR *>&this->_service, sizeof(SOCKADDR)) == SOCKET_ERROR)
  {
    std::cerr << "/!\\ Bind error. /!\\" << std::endl;
    closesock(this->_sock);
  }
}
...
WinTcpSock	&WinTcpSock::operator=(const WinTcpSock &cpy)
{
  this->_ip = cpy.getIp();
...
WinTcpSock::WinTcpSock(const WinTcpSock &cpy)
{
  *this = cpy;
}

WinTcpSock::WinTcpSock(const std::string &ip, short port)
  : _ip(ip), _port(port)
{
